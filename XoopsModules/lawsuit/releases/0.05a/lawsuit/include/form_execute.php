	...
	foreach( $attachments as $a ){
		if( false == $xoopsMailer->isMail || $a['saveto'] ){
			...
			if( false == $uploader[$a['id']]->upload() ){
				...
				$msg[$a['id']] .= sprintf(_LAWSUIT_UPLOADED_FILE, strtolower(substr($_SERVER['SERVER_PROTOCOL'], 0, strpos($_SERVER['SERVER_PROTOCOL'], "/", 0)))."://".$_SERVER['HTTP_HOST'].LAWSUIT_URL.'admin/file.php?f='.$saved);
			}
		}else{
