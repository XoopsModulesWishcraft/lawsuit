...
class LawsuitCategory extends XoopsObject {
	...
	function getURL() {
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
			return XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['baseurl'].'/'.xoops_sef($this->getVar('title')).'/'.$this->getVar('cid').$GLOBALS['xoopsModuleConfig']['endofurl'];
		} else {
...
class LawsuitCategoryHandler extends XoopsObjectHandler {
	...
	function &getInstance(&$db){
		static $instance;
		if( !isset($instance) ){
			$instance = new LawsuitCategoryHandler($db);
		}
		return $instance;
	}
	function &create(){
