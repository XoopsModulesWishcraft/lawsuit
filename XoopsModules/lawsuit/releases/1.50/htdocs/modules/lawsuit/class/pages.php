...
class LawsuitPages extends XoopsObject {
	...
	function getURL() {
		...
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
			return XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['baseurl'].'/'.xoops_sef($category[$this->getVar('cid')]->getVar('title')).'/'.xoops_sef($this->getVar('title')).'/'.$this->getVar('pid').$GLOBALS['xoopsModuleConfig']['endofurl'];
		} else {
...
class LawsuitPagesHandler extends XoopsObjectHandler {
	...
	function &getInstance(&$db){
		static $instance;
		if( !isset($instance) ){
			$instance = new LawsuitPagesHandler($db);
		}
		return $instance;
	}
	function &create(){
	...
	function insert(&$page, $force = false){
		...
		For some reason both instanciations of this code including is_a is 0!=0
