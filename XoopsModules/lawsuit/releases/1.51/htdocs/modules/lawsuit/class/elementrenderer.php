...
class LawsuitElementRenderer{
	...
	function __construct(&$element) {
		return $this->LawsuitElementRenderer(&$element);
	}
		...
		switch($e){
			case 'text':
				if( !is_object($GLOBALS['xoopsUser']) ){
					...
					$ele_value[2] = str_replace('{UNAME}', $GLOBALS['xoopsUser']->getVar('uname', 'e'), $ele_value[2]);
					$ele_value[2] = str_replace('{EMAIL}', $GLOBALS['xoopsUser']->getVar('email', 'e'), $ele_value[2]);
				}
	...
	function optOther($s='', $id){
		...
		$len = !empty($s[1]) ? $s[1] : $GLOBALS['lawsuitModuleConfig']['t_width'];
