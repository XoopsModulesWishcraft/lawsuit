...
switch($op){
	...
	case 'list':
		...
		foreach($responses as $responseid => $response){		
			...
			$i=0;
			foreach($response->getResponse() as $key => $res) {
				$i++;
				if ($i<=2) {
					if (is_array($res)) {
					...
					foreach($res as $keyb => $value) {
						if (is_array($value)) {
							...
							foreach($value as $keyc => $valueb) {
								...
								$class=($class!='even')?'even':'odd';
								echo '<tr class="'.$class.'">
											<td align="right">'.($j<2?$key.':&nbsp;':'&nbsp;').'</td>
											<td align="left"><em>'.$valueb.'</em></td>
									 </tr>';
							}
						} else {
							$class=($class!='even')?'even':'odd';
