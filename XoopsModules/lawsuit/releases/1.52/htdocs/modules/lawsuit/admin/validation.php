...
switch($op){
	...
	case 'list':
		adminHtmlHeader();
		lawsuit_adminMenu('validation.php', 6);
	...
	case 'delete':
		if( empty($_POST['ok']) ){
			xoops_cp_header();
			lawsuit_adminMenu('validation.php', 6);
			xoops_confirm(array('op' => 'delete', 'rule_id' => $_GET['rule_id'], 'ok' => 1), $_SERVER['PHP_SELF'], _AM_FORM_CONFIRM_DELETE);
		}else{
			...
			redirect_header($_SERVER['PHP_SELF'], 0, _AM_DBUPDATED);
		}
	break;
	...
	case 'saveform':
		...
		if( !empty($error) ){
			...
			redirect_header($_SERVER['PHP_SELF'].'?op=edit&amp;rule_id='.$ret, 0, _AM_DBUPDATED);
		}
	break;
}
