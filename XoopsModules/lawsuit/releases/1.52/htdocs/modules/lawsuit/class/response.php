...
class LawsuitResponse extends XoopsObject {
	...
	function getResponse() {
		return lawsuit_object2array(json_decode($this->getVar('response')));
	}
}
...
class LawsuitResponseHandler extends XoopsPersistableObjectHandler {
	...
	function insert($object, $force=true) {
		$object->setVar('time_response', time());
		$object->setVar('fingerprint', md5($object->getVar('response').$object->getVar('time_response')));
