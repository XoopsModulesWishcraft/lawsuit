<?php
// $Id: form_execute.php,v 1.05 2009/06/24 23:45:00 wishcraft Exp $
...
if (!$xoopsCaptcha->verify()) {
	redirect_header(XOOPS_URL.'/modules/lawsuit/', 4, $xoopsCaptcha->getMessage());
	exit(0);
}
...
$msg = $err = $attachments = $resp = array();
	...
	if( isset($ele[$ele_id]) ){
		...
		switch($ele_type){
			...
			case 'text':
				...
				$resp[$ele_caption][$ele_id] = $myts->stripSlashesGPC($ele[$ele_id]);
			break;
			case 'textarea':
				$msg[$ele_id] .= $myts->stripSlashesGPC($ele[$ele_id]);
				$resp[$ele_caption][$ele_id] = $myts->stripSlashesGPC($ele[$ele_id]);
				...
				while( $v = each($ele_value) ){
					if( $opt_count == $ele[$ele_id] ){
						...
						if( $other != false ){
							...
							$resp[$ele_caption][$ele_id] = $myts->stripSlashesGPC($v['key']);
						}
					}
					$opt_count++;
				}
			...
			case 'yn':
				...
				$resp[$ele_caption][$ele_id] = $myts->stripSlashesGPC($v);
			break;
			case 'checkbox':
				$opt_count = 1;
				$ch = array();
...
if( is_dir(LAWSUIT_ROOT_PATH."language/".$GLOBALS['xoopsConfig']['language']."/mail_template") ){
	$template_dir = LAWSUIT_ROOT_PATH."language/".$GLOBALS['xoopsConfig']['language']."/mail_template";
}else{
...
if( in_array('user', $xoopsModuleConfig['moreinfo']) ){
	if( is_object($xoopsUser) ){
		...
		$resp["UNAME"][0] = sprintf(_LAWSUIT_MSG_UNAME, $xoopsUser->getVar("uname"));
		$resp["ULINK"][0] = sprintf(_LAWSUIT_MSG_UINFO, XOOPS_URL.'/userinfo.php?uid='.$xoopsUser->getVar("uid"));
	}else{
		$xoopsMailer->assign("UNAME", sprintf(_LAWSUIT_MSG_UNAME, $GLOBALS['xoopsConfig']['anonymous']));
		$xoopsMailer->assign("ULINK", '');
		$resp["UNAME"][0] = sprintf(_LAWSUIT_MSG_UNAME, $GLOBALS['xoopsConfig']['anonymous']);
		$resp["ULINK"][0] = sprintf(_LAWSUIT_MSG_UINFO, XOOPS_URL.'/userinfo.php?uid=0');
	}
}else{
...
if( in_array('ip', $xoopsModuleConfig['moreinfo']) ){
	...
	if( $proxy != $ip ){
		$ip = $ip.sprintf(_LAWSUIT_PROXY, $proxy);
	}
	$resp["IP"] = $ip;
...
if( in_array('form', $xoopsModuleConfig['moreinfo']) ){
	...
	$resp["FORMURL"] = $_REQUEST['PHP_SELF'];
}
	...
	foreach( $attachments as $a ){
		if( false == $xoopsMailer->isMail || $a['saveto'] ){
			...
			if( false == $uploader[$a['id']]->upload() ){
				...
				$msg[$a['id']] .= sprintf(_LAWSUIT_UPLOADED_FILE, LAWSUIT_URL.'admin/file.php?f='.$saved);
			}
		}else{
...
$response->setVar('response', json_encode($resp));
$refid = $lawsuit_response_mgr->insert($response);
...
if ($refid>0) {
	$xoopsOption['template_main'] = 'lawsuit_reference.html';
	include_once XOOPS_ROOT_PATH.'/header.php';
	$xoopsTpl->assign('referenceid', $refid);
	include XOOPS_ROOT_PATH.'/footer.php';
	exit();
}
